...
void convert_to_normal_file(struct inode *ip){
  uint tot, m, off = 0;
  struct buf *bp;
...
int writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  // todo : change this if you want to convert a small file into a normal file
  ...
  if (ip->type == T_SFILE)
  {
    // cprintf("Writing to SFILE with off : %d, n : %d, data : %s\n", off, n, src);
    memmove((char *)(ip->addrs) + off, src, n);
    off += n;
  }
  else
  {
    // cprintf("Writing to normal FILE with off : %d, n : %d, data : %s\n", off, n, src);
